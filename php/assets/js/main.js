...
$(document).ready(function () {
  if ($('#countryTypeahed').length > 0 && typeof countryList !== 'undefined' && countryList.length > 0) {
    if ($('#countryValue').val() !== '') {
      $.each(countryList , function(id, details) {
        if ('selected' in details && details.selected === true) {
          $('#countryTypeahed').val(details.name);
          $('input[name*=phone_index]').val(details.phoneIndex);
          $('span.phone-index').html(details.phoneIndex);
        }
      });
    }
    $('#countryTypeahed').typeahead({
      source: countryList,
      minLength: 1,
      showHintOnFocus: true,
      afterSelect: function(current){
        $('span.phone-index').html(current.phoneIndex);
        $('input[name*=phone_index]').val(current.phoneIndex);
        $('#countryValue').val(current.id);
      },
      delay: 300,
      autoSelect: true
    });
  ...
  if ($('#organisationSearch').length > 0 && typeof companyList !== 'undefined' && companyList.length > 0) {
    $('#organisationSearch').typeahead({
      source: companyList,
      minLength: 1,
      showHintOnFocus: true,
      afterSelect: function(current){
        $.ajax({
          url: '/register',
          type: 'POST',
          data: {ajax: true, comp_id: current.id, getorg: true},
          success: function(resp){
            $("#compPreview").html("<tr><th>Organization Name</th><th>Address</th></tr>"+
              "<tr><td>"+resp.name+"</td><td>"+resp.address[0].streets[0]+
              ", "+resp.address[0].state+", "+resp.address[0].country+"</td></tr>")
              .show();
            $("#organisationSearchValue").val(resp.id);
          }
        });
      },
      delay: 300,
  ...
  $("input, textarea, select").on('change', function(){
    $(this).parent().closest(".help-block").remove();
